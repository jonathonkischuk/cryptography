...
class ElipticCurveCryptography:
    def __init__(self, a, b):
        # y^2 = x^3 + ax + b
        # Ex. Bitcoin: a = 0 and b = 7 --> (y^2 = x^3 + 7)
        self.a = a
        self.b = b
...
if __name__ == '__main__':
    ecc = ElipticCurveCryptography(0, 7)
    # the E elliptic curve + the G generator is public
    generator_point = Point(-2, -1)

    # Alice random number (a)
    alice_random = random.randint(2, int(1e4))
    # Bob random number (b)
    bob_random = random.randint(2, int(1e4))
